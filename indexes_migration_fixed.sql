-- Phase 2: Data Integrity & Performance Indexes (Fixed)
...
-- Note: These tables use 'stage_id', not 'pipeline_stage_id'
CREATE INDEX IF NOT EXISTS idx_automation_rules_stage 
ON public.automation_rules (stage_id);

CREATE INDEX IF NOT EXISTS idx_stage_obligations_stage 
ON public.stage_obligations (stage_id);
...
-- Check if this one exists with a different name or create it

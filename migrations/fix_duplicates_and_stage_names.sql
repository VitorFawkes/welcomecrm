-- ============================================================================
-- CORREÇÃO FINAL DE DUPLICIDADE E NOMES DE ETAPAS
-- ============================================================================

-- 1. Remover TODOS os triggers possíveis que podem estar causando duplicidade
...
drop trigger if exists update_card_trigger on cards;
drop trigger if exists audit_card_trigger on cards;
drop trigger if exists log_card_update on cards;
drop trigger if exists card_log_trigger on cards;
drop trigger if exists trigger_log_card_changes on cards;
drop trigger if exists on_card_update on cards;
drop trigger if exists handle_card_update on cards;

-- 2. Atualizar a função para buscar nomes das etapas e fases
...
begin
    ...
    -- Stage change (COM BUSCA DE NOME)
...
-- Verificação
select 'Triggers limpos e função atualizada com nomes das etapas!' as resultado;

-- ============================================================================
-- CORREÇÃO: Comparação de strings vazias e null em JSONB
-- ============================================================================
...
begin
    ...
    if OLD.pipeline_stage_id is distinct from NEW.pipeline_stage_id then
        select nome into old_stage_name from pipeline_stages where id = OLD.pipeline_stage_id;
        select nome into new_stage_name from pipeline_stages where id = NEW.pipeline_stage_id;
        
        insert into activities (card_id, tipo, descricao, metadata, created_by)
        values (NEW.id, 'stage_changed', 
            'Etapa: "' || coalesce(old_stage_name, 'Desconhecida') || '" → "' || coalesce(new_stage_name, 'Desconhecida') || '"',
            jsonb_build_object(
    ...
    -- Motivo da Viagem
    if coalesce(old_produto_data->>'motivo', '') is distinct from coalesce(new_produto_data->>'motivo', '') then
        insert into activities (card_id, tipo, descricao, metadata, created_by)
        values (NEW.id, 'updated', 
    ...
    -- Origem do Lead
    if coalesce(old_produto_data->>'origem_lead', '') is distinct from coalesce(new_produto_data->>'origem_lead', '') then
        insert into activities (card_id, tipo, descricao, metadata, created_by)
        values (NEW.id, 'updated', 
    ...
    if coalesce(old_produto_data->>'observacoes', '') is distinct from coalesce(new_produto_data->>'observacoes', '') then
    ...
    if coalesce(old_obs_criticas->>'o_que_e_importante', '') is distinct from coalesce(new_obs_criticas->>'o_que_e_importante', '') then
    ...
    if coalesce(old_obs_criticas->>'o_que_nao_pode_dar_errado', '') is distinct from coalesce(new_obs_criticas->>'o_que_nao_pode_dar_errado', '') then
    ...
    -- Sensibilidades
    if coalesce(old_obs_criticas->>'sensibilidades', '') is distinct from coalesce(new_obs_criticas->>'sensibilidades', '') then
    ...
    -- Status da Taxa
    if coalesce(old_taxa->>'status', '') is distinct from coalesce(new_taxa->>'status', '') then
    ...
    -- Valor da Taxa
    if coalesce(old_taxa->>'valor', '0') is distinct from coalesce(new_taxa->>'valor', '0') then
        insert into activities (card_id, tipo, descricao, metadata, created_by)
        values (NEW.id, 'value_changed', 

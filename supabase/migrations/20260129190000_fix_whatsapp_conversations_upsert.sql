-- ============================================================
-- MIGRATION: Fix WhatsApp Conversations UPSERT
-- Date: 2026-01-29
-- Description: Restaura o UPSERT na tabela whatsapp_conversations
--              que estava faltando na funcao process_whatsapp_raw_event_v2
-- ============================================================

-- A funcao atual no banco nao tem o UPSERT para whatsapp_conversations
-- Isso faz com que o link do WhatsApp no card va para wa.me ao inves do Echo
...
BEGIN
    ...
    -- ============================================================
    -- 12. UPSERT CONVERSATION TRACKING (THIS WAS MISSING!)
    -- ============================================================
    IF v_conversation_id IS NOT NULL THEN
...
-- ============================================================
-- BACKFILL: Populate whatsapp_conversations from existing messages
-- ============================================================
...
SELECT DISTINCT ON (m.contact_id, m.platform_id)
    m.contact_id,
    m.platform_id,
    m.conversation_id as external_conversation_id,
    'https://echo-wpp.vercel.app/dashboard/' || m.conversation_id as external_conversation_url,
...
GROUP BY m.contact_id, m.platform_id, m.conversation_id
ON CONFLICT (contact_id, platform_id) WHERE platform_id IS NOT NULL
DO UPDATE SET
    external_conversation_id = EXCLUDED.external_conversation_id,
    external_conversation_url = COALESCE(EXCLUDED.external_conversation_url, whatsapp_conversations.external_conversation_url),
...
COMMENT ON FUNCTION process_whatsapp_raw_event_v2 IS 'Processes WhatsApp raw events. Fixed: Added UPSERT for whatsapp_conversations table.';

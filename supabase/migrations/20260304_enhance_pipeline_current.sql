-- ============================================================
-- Enhance analytics_pipeline_current:
-- 1. aging CTE: add fase_slug (para filtro client-side por fase)
-- 2. top_deals CTE: add fase_slug
-- 3. owners CTE: add by_phase_value (valor por fase)
-- ============================================================
...
BEGIN
    ...
    -- Aging heatmap (now with fase_slug for client-side phase filtering)
    aging AS (
        SELECT jsonb_agg(row_data ORDER BY fase_order, ordem) AS val
        FROM (
            SELECT
    ...
    -- Carga por consultor (now with by_phase_value)
    owners AS (
    ...
    -- Top deals em risco (now with fase_slug)
    top_deals AS (

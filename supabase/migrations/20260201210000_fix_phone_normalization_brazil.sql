-- ============================================================
-- MIGRATION: Fix Phone Normalization for Brazil Country Code
-- Date: 2026-02-01
-- Description: Corrige a normalização de telefone para lidar com
--              código do país 55 (Brasil) que vem do Echo/WhatsApp
-- Issue: Telefone no CRM: (31) 99146-1394 -> 31991461394
--        Telefone do Echo: 5531991461394 (com código 55)
--        Busca falhava porque normalize_phone não remove o 55
-- ============================================================

-- 1. Criar função auxiliar que remove código do país Brasil (55)
...
BEGIN
    ...
    -- Se começa com 55 e tem mais de 10 dígitos, remove o 55
    -- Telefone BR válido: 10-11 dígitos (DDD + número)
...
COMMENT ON FUNCTION normalize_phone_brazil IS 'Normaliza telefone removendo código do país 55 do Brasil';

-- 2. Atualizar a função process_whatsapp_raw_event_v2 para usar busca mais robusta
...
DECLARE
    ...
    v_phone_no_country TEXT;  -- Telefone sem código do país
    v_contact_id UUID;
    v_card_id UUID;
    v_message_id UUID;
    v_profile_id UUID;
...
BEGIN
    ...
    SELECT (value = 'true') INTO v_processing_enabled
    FROM integration_settings
    WHERE key = 'WHATSAPP_PROCESS_ENABLED';

    IF v_processing_enabled IS NULL OR v_processing_enabled = false THEN
    ...
    -- 5. Normalize phone - AGORA COM SUPORTE A CÓDIGO DO PAÍS
    v_phone_normalized := normalize_phone(v_phone);
    v_phone_no_country := normalize_phone_brazil(v_phone);  -- Remove o 55 se presente
    ...
    -- 7. Find contact via contato_meios (BUSCA ROBUSTA)
    -- Tenta com o telefone completo OU sem código do país
    SELECT cm.contato_id INTO v_contact_id
    FROM contato_meios cm
    WHERE cm.tipo IN ('telefone', 'whatsapp')
    AND (
        cm.valor_normalizado = v_phone_normalized
        OR cm.valor_normalizado = v_phone_no_country
        OR normalize_phone_brazil(cm.valor_normalizado) = v_phone_no_country
    )
    LIMIT 1;

    -- Fallback: search in contatos.telefone (BUSCA ROBUSTA)
    IF v_contact_id IS NULL THEN
        SELECT id INTO v_contact_id
        FROM contatos
        WHERE normalize_phone_brazil(telefone) = v_phone_no_country
        LIMIT 1;

        IF v_contact_id IS NOT NULL THEN
            -- Auto-register in contato_meios for future lookups
    ...
    IF v_contact_id IS NULL THEN
        SELECT (value = 'true') INTO v_create_contact_enabled
        FROM integration_settings
    ...
    SELECT (value = 'true') INTO v_link_to_card_enabled
    FROM integration_settings
    ...
    IF v_link_to_card_enabled = true THEN
        ...
        AND c.status_comercial NOT IN ('won', 'lost')
        AND c.deleted_at IS NULL
        ORDER BY c.created_at DESC
        LIMIT 1;
    END IF;

    -- 10. Resolve agent to profile
    ...
    -- 12. UPSERT CONVERSATION TRACKING
    IF v_conversation_id IS NOT NULL THEN
    ...
    RETURN jsonb_build_object(
        ...
        'conversation_url', v_conversation_url,
        'phone', v_phone_normalized,
        'phone_no_country', v_phone_no_country,
...
COMMENT ON FUNCTION process_whatsapp_raw_event_v2 IS 'Processes WhatsApp raw events. v3: Adds robust phone matching with Brazil country code (55) handling.';

-- ============================================================
-- 3. REPROCESS: Eventos que falharam por "no_contact" com código 55
-- ============================================================

-- Marca eventos no_contact que podem ser reprocessados agora
UPDATE whatsapp_raw_events
SET status = 'pending', error_message = NULL, processed_at = NULL
WHERE status = 'no_contact'
AND raw_payload::text LIKE '%5531%'
AND created_at > NOW() - INTERVAL '30 days';

-- Log de quantos eventos serão reprocessados
DO $$
DECLARE
    v_count INTEGER;
BEGIN
    ...
    RAISE NOTICE 'Eventos marcados para reprocessamento: %', v_count;
END $$;

-- Migration: fix_cards_schema_and_cleanup
-- Description: Fixes the cards.pessoa_principal_id foreign key to reference contatos instead of pessoas, migrates data, and drops the legacy pessoas table.

-- 1. Ensure all referenced people exist in contatos (Migration)
INSERT INTO contatos (id, nome, email, created_at, updated_at)
SELECT 
    p.id, 
    p.nome, 
    p.email,
...
FROM pessoas p
WHERE p.id IN (SELECT pessoa_principal_id FROM cards WHERE pessoa_principal_id IS NOT NULL)
AND p.id NOT IN (SELECT id FROM contatos)
ON CONFLICT (id) DO NOTHING;

-- 2. Drop the old foreign key constraint referencing pessoas
ALTER TABLE cards DROP CONSTRAINT IF EXISTS cards_pessoa_principal_id_fkey;

-- 3. Add the new foreign key constraint referencing contatos
ALTER TABLE cards ADD CONSTRAINT cards_pessoa_principal_id_fkey 
    FOREIGN KEY (pessoa_principal_id) REFERENCES contatos(id);

-- 4. Drop the legacy 'pessoas' table
-- WARNING: This is a destructive operation. Ensure all data is migrated.
DROP TABLE IF EXISTS pessoas CASCADE;

-- 5. Drop views that might depend on view_cards_acoes or be affected by changes
DROP VIEW IF EXISTS view_dashboard_funil;
DROP VIEW IF EXISTS view_cards_acoes CASCADE;

-- 6. Recreate view_cards_acoes

-- ============================================================================
-- MIGRATION: Simplificar taxonomia de origem de leads
-- Problem: cards.origem tem valores técnicos sem visibilidade. Contatos sem rastreamento.
-- Solution: Simplificar constraint, migrar dados, adicionar origem em contatos,
--           expor origem_lead na view_cards_acoes
-- Date: 2026-02-12
-- Protocol: SQL_SOP followed — view definition based on 20260211120000 (latest)
-- ============================================================================

BEGIN;

-- ============================================================================
-- 1. MIGRATE LEGACY ORIGIN VALUES
-- ============================================================================
...
COMMENT ON CONSTRAINT cards_origem_check ON cards IS
    'Simplified origin taxonomy. Manual: mkt, indicacao, carteira, manual, outro. Auto: site, active_campaign, whatsapp. Updated 2026-02-12.';

-- ============================================================================
-- 3. ADD ORIGIN TRACKING TO CONTATOS
-- ============================================================================

ALTER TABLE contatos ADD COLUMN IF NOT EXISTS origem TEXT;
ALTER TABLE contatos ADD COLUMN IF NOT EXISTS origem_detalhe TEXT;

COMMENT ON COLUMN contatos.origem IS 'How this contact first entered the CRM (mkt, indicacao, carteira, etc.)';
COMMENT ON COLUMN contatos.origem_detalhe IS 'Detail about the origin: who referred, which campaign, etc.';

-- ============================================================================
-- 4. INDEX FOR FILTER PERFORMANCE
-- ============================================================================

CREATE INDEX IF NOT EXISTS idx_cards_origem ON cards(origem) WHERE deleted_at IS NULL;

-- ============================================================================
-- 5. REBUILD view_cards_acoes with c.origem_lead added
--    Base: migration 20260211120000 (latest live definition)
-- ============================================================================
...
SELECT
    ...
    c.marketing_data,
    c.origem,
    c.origem_lead,  -- NEW: detail about origin (who referred, campaign, etc.)
    c.external_id,
    c.campaign_id,
    c.moeda,
    ...
    normalize_phone_brazil(pe.telefone) AS pessoa_telefone_normalizado,
...
-- ============================================================================
-- 6. RECREATE view_dashboard_funil (unchanged)
-- ============================================================================
...
-- ============================================================================
-- 7. RECREATE view_archived_cards (unchanged)
-- ============================================================================

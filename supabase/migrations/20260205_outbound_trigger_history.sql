-- =====================================================
-- OUTBOUND TRIGGER HISTORY SYSTEM
-- Adiciona rastreamento de qual regra processou cada evento outbound
-- Espelho do sistema implementado para inbound
-- =====================================================

-- 1. Adicionar coluna matched_trigger_id na fila de outbound
ALTER TABLE public.integration_outbound_queue
ADD COLUMN IF NOT EXISTS matched_trigger_id UUID REFERENCES public.integration_outbound_triggers(id);

-- 2. Criar índice para queries eficientes por trigger
CREATE INDEX IF NOT EXISTS idx_outbound_queue_trigger
ON public.integration_outbound_queue(matched_trigger_id)
WHERE matched_trigger_id IS NOT NULL;

-- 3. RPC para estatísticas por trigger (outbound)
CREATE OR REPLACE FUNCTION get_outbound_trigger_event_stats(p_integration_id UUID)
RETURNS TABLE(trigger_id UUID, status TEXT, cnt BIGINT)
LANGUAGE sql STABLE
AS $$
  SELECT
    matched_trigger_id as trigger_id,
    status,
    count(*) as cnt
  FROM public.integration_outbound_queue
  WHERE integration_id = p_integration_id
  GROUP BY matched_trigger_id, status
  ORDER BY trigger_id, status;
$$;

COMMENT ON FUNCTION get_outbound_trigger_event_stats IS 'Retorna contagem de eventos outbound agrupados por trigger e status';

-- 4. Atualizar função log_outbound_card_event para salvar matched_trigger_id e registrar eventos bloqueados
...
-- 5. Comentário na nova coluna
COMMENT ON COLUMN public.integration_outbound_queue.matched_trigger_id IS 'ID da regra de outbound que processou este evento (allow ou block)';

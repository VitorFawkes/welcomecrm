-- ============================================================================
-- MIGRATION: Add Revenue (Receita) Tracking
-- Description: Adds supplier_cost to proposal items/flights, receita to cards,
--              updates views, creates trigger for auto-calculation
-- Date: 2026-02-05
-- ============================================================================
...
-- ============================================================================
-- 1. ADD SUPPLIER COST TO PROPOSAL ITEMS
-- ============================================================================

ALTER TABLE proposal_items
  ADD COLUMN IF NOT EXISTS supplier_cost DECIMAL(12, 2) DEFAULT 0;

COMMENT ON COLUMN proposal_items.supplier_cost IS 'Custo do fornecedor para este item. Receita = base_price - supplier_cost';

-- ============================================================================
-- 2. ADD SUPPLIER COST TO PROPOSAL FLIGHTS
-- ============================================================================

ALTER TABLE proposal_flights
  ADD COLUMN IF NOT EXISTS supplier_cost DECIMAL(12, 2) DEFAULT 0;

COMMENT ON COLUMN proposal_flights.supplier_cost IS 'Custo do fornecedor para este voo. Receita = price_total - supplier_cost';

-- ============================================================================
-- 3. ADD RECEITA FIELDS TO CARDS
-- ============================================================================

ALTER TABLE cards
  ADD COLUMN IF NOT EXISTS receita DECIMAL(12, 2),
  ADD COLUMN IF NOT EXISTS receita_source TEXT DEFAULT 'manual';

-- Add check constraint separately for idempotency
DO $$ BEGIN
  ALTER TABLE cards
    ADD CONSTRAINT chk_receita_source
    CHECK (receita_source IN ('calculated', 'manual'));
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;

COMMENT ON COLUMN cards.receita IS 'Receita/margem da agência. Calculada (faturamento - custo) ou informada manualmente';
COMMENT ON COLUMN cards.receita_source IS 'Fonte do valor: calculated (via proposta) ou manual (digitado)';

-- ============================================================================
-- 4. INDEXES
-- ============================================================================

CREATE INDEX IF NOT EXISTS idx_proposal_items_supplier_cost
  ON proposal_items(section_id, supplier_cost);

CREATE INDEX IF NOT EXISTS idx_cards_receita
  ON cards(receita) WHERE receita IS NOT NULL;

-- ============================================================================
-- 5. TRIGGER: Sync proposal revenue to card on acceptance
-- ============================================================================
...
BEGIN
  ...
  IF NEW.status::text = 'accepted' AND (OLD.status IS NULL OR OLD.status::text != 'accepted') THEN
    ...
    -- Total receita
    v_total_receita := (v_items_faturamento + v_flights_faturamento)
                     - (v_items_custo + v_flights_custo);

    -- Update card only if receita_source is not 'manual' (respect manual overrides)
    UPDATE cards
    SET
      ...
      updated_at = NOW()
    WHERE id = v_card_id
      AND (receita_source IS NULL OR receita_source != 'manual');
  END IF;
...
-- ============================================================================
-- 6. RPC: Recalculate receita for a card (from accepted proposal)
-- ============================================================================
...
-- ============================================================================
-- 7. UPDATE VIEWS (DROP CASCADE + RECREATE)
-- ============================================================================
...
-- 7a. Recreate view_cards_acoes with receita
...
-- 7b. Recreate view_dashboard_funil with receita
...
-- 7c. Recreate view_archived_cards with receita
...
ANALYZE proposal_items;
ANALYZE proposal_flights;
ANALYZE cards;

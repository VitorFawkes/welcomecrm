-- ============================================================
-- MIGRATION: Fix WhatsApp Auto-Processing (FINAL)
-- Date: 2026-01-23
-- Description: Fixes processing function bugs and adds auto-trigger
-- ============================================================

-- ═══════════════════════════════════════════════════════════════════════════
-- PART 1: FIX THE PROCESSING FUNCTION
-- Fixed: boolean toggles, status_comercial column, removed produto filter
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    -- Fallback: search in contatos.telefone
    ...
    -- 9. Find active card for this contact (uses status_comercial)
...
-- ═══════════════════════════════════════════════════════════════════════════
-- PART 2: CREATE AUTO-PROCESSING TRIGGER
-- ═══════════════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION trigger_auto_process_whatsapp_event()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
    v_result jsonb;
BEGIN
    v_result := process_whatsapp_raw_event_v2(NEW.id);
    
    IF v_result->>'error' IS NOT NULL THEN
        RAISE NOTICE 'WhatsApp event processing error: %', v_result->>'error';
    END IF;
...
DROP TRIGGER IF EXISTS trg_auto_process_whatsapp_event ON whatsapp_raw_events;

CREATE TRIGGER trg_auto_process_whatsapp_event
    AFTER INSERT ON whatsapp_raw_events
    FOR EACH ROW
    EXECUTE FUNCTION trigger_auto_process_whatsapp_event();

-- ═══════════════════════════════════════════════════════════════════════════
-- PART 3: ENSURE REALTIME IS ENABLED FOR WHATSAPP_MESSAGES
-- ═══════════════════════════════════════════════════════════════════════════
...
COMMENT ON FUNCTION process_whatsapp_raw_event_v2 IS 'Processes WhatsApp raw events. Fixed: boolean toggles, status_comercial column.';
COMMENT ON TRIGGER trg_auto_process_whatsapp_event ON whatsapp_raw_events IS 'Auto-processes WhatsApp events on insert';

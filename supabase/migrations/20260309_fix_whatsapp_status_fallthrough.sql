-- ============================================================
-- MIGRATION: Fix WhatsApp message.status fallthrough
-- Date: 2026-03-09
--
-- Problema:
-- Linhas como "Mariana Volpi" e "Trips - Travel Planner" enviam
-- mensagens outbound como eventos message.status (não message.sent).
-- O short-circuit atual tenta UPDATE por external_id, mas se a
-- mensagem nunca foi salva antes (rows_updated=0), ela é descartada
-- silenciosamente.
--
-- Fix:
-- Quando message.status não encontra mensagem existente (0 rows),
-- em vez de retornar, cai no pipeline completo (contato → card →
-- INSERT) para salvar a mensagem pela primeira vez.
-- ============================================================
...
BEGIN
    ...
    -- message.status → tenta atualizar status da mensagem existente.
    -- Se não encontra (rows_updated=0), cai no pipeline completo
    -- para salvar a mensagem pela primeira vez.
    -- ========================================
    IF v_platform.provider = 'echo' AND (v_payload->>'event') = 'message.status' AND v_external_id IS NOT NULL THEN
        -- Extrair status do payload Echo
        ...
        -- Tentar atualizar mensagem existente pelo external_id
        ...
        -- Se encontrou mensagem existente, short-circuit normal
        ...
        -- Se NÃO encontrou (rows_updated=0), cai no pipeline completo
        -- para salvar a mensagem pela primeira vez.
        -- Isto acontece quando linhas enviam outbound como message.status
        -- em vez de message.sent (ex: Mariana Volpi, Trips - Travel Planner).
    END IF;

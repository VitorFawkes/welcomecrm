...
DROP POLICY IF EXISTS "Admins can view audit logs" ON public.audit_logs; -- Depends on is_admin which depends on role
...
-- 7. Recreate Policies using new functions
...
-- PF Data
CREATE POLICY "PF Data viewable by privileged or owner" ON public.dados_cadastrais_pf
    FOR SELECT USING (public.is_gestor() OR (created_by = auth.uid()));

CREATE POLICY "PF Data modify by privileged" ON public.dados_cadastrais_pf
    FOR ALL USING (public.is_gestor());

-- PJ Data
CREATE POLICY "PJ Data viewable by privileged or owner" ON public.dados_cadastrais_pj
    FOR SELECT USING (public.is_gestor() OR (created_by = auth.uid()));

CREATE POLICY "PJ Data modify by privileged" ON public.dados_cadastrais_pj
    FOR ALL USING (public.is_gestor());

-- Contratos
CREATE POLICY "Contratos modify by privileged or owner" ON public.contratos
    FOR ALL USING (public.is_gestor() OR (created_by = auth.uid()));
